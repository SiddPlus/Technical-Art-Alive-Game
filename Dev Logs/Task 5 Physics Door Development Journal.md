...
I created an actor for a physics-based door mechanism, starting with the core visual components. To build the structure, I added two static meshes: one representing the door itself and the other acting as the door frame. These meshes served as the primary visual elements, providing a defined space for the door to open and close within. I then attached box collision components to both the door and the frame, enabling player interaction by detecting collisions with the door.

Next, I configured the door’s movement through a physics constraint, which controlled the door’s rotation and interaction with physics forces. I set the constraint’s Z rotation to -90 degrees to ensure the door would rotate around a realistic hinge point. Naming the constraint component “door,” I fine-tuned its swing and twist motions to mimic a natural door swing: Swing Motion 1 was set to Limited, Swing Motion 2 to Locked, and Twist Motion to Locked. This setup restricted the door’s movement to a single rotational axis, simulating the typical motion of a hinged door. Additionally, I set a 90-degree swing limit and selected Twist and Swing as the angular drive mode, controlling the door’s open-and-close range.

To finalize the interaction, I adjusted the target orientation, setting the Y-axis to 90 degrees and enabling both twist and swing drives. With these settings, the door would respond realistically to player collisions, swinging open on impact, then naturally oscillating before coming to a gradual stop. This carefully crafted mechanism provided an interactive experience where the door opened smoothly, responding to the player’s movement and behaving as a real door would, adding to the scene’s immersion.
